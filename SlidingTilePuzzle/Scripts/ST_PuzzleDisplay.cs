...
public class ST_PuzzleDisplay : MonoBehaviour
{
    ...
    // where empty part of puzzle.
    public int EmptySpace = 0;

    public int timeSpan = 0;
    private int[,] tilePlace = { { 0, 3, 6 }, { 1, 4, 7 }, { 2, 5, 8 } };
    ...
    void Update()
    {
        ...
        if (timeSpan == 0)
        {
            if (Input.GetKey(KeyCode.LeftArrow))
            {
                ...
                if (EmptySpace % 3 != 2)
                {
                    int tmp = tilePlace[EmptySpace % 3 + 1, EmptySpace / 3];
                    TileDisplayArray[tmp % 3, tmp / 3].GetComponent<ST_PuzzleTile>().ExecuteAdditionalMove();
                    timeSpan = 30;
                }
            }
            ...
            else if (Input.GetKey(KeyCode.DownArrow))
            {
                ...
                if (EmptySpace / 3 != 2)
                {
                    int tmp = tilePlace[EmptySpace % 3, EmptySpace / 3 + 1];
                    TileDisplayArray[tmp % 3, tmp / 3].GetComponent<ST_PuzzleTile>().ExecuteAdditionalMove();
                    timeSpan = 30;
                }
            }
